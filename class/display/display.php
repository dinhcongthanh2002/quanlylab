	...
	*/
	class display
	{
				...
				foreach ($fetch as $key => $value) {
					...
					$button1 = 	'<div class="btn-group">
									...
									<ul class="dropdown-menu">
										<li><a href="javascript:;" class="edit-room" ><i class="fa fa-edit"></i> Sửa</a></li>
										<li><a href="room_info?name='.$myname.'&id='.$value["id"].'"><i class="fa fa-search"></i> Xem</a></li>
									</ul>
								</div>';
		...
		public function display_member()
		{
			...
			if($count > 0){
				foreach ($fetch as $key => $value) {
					$status = ($value['m_status'] == 1) ? '<a href="javascript:;" class="deactivate-member" ><i class="fa fa-remove"></i> tắt</a>' : '<a href="javascript:;" class="activate-member" ><i class="fa fa-remove"></i> kích hoạt</a>' ;
					$button = 	'<div class="btn-group">
									<button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
										Action <span class="caret"></span>
									</button>
									<ul class="dropdown-menu">
										<li><a href="javascript:;" class="edit-member" ><i class="fa fa-edit"></i> Sửa</a></li>
										<li>'.$status.'</li>
										<li><a href="member_profile?id='.$value['id'].'&name='.$value['m_fname'].' '.$value['m_lname'].'"><i class="fa fa-user"></i> Hồ sơ người mượn</a></li>
									</ul>
								</div>';
		...
		public function display_user()
		{
			...
			if($count > 0){
				foreach ($fetch as $key => $value) {
					$status = ($value['status'] == 1) ? '<a href="javascript:;" class="deactivate-user" ><i class="fa fa-remove"></i> tắt</a>' : '<a href="javascript:;" class="activate-user" ><i class="fa fa-remove"></i> kích hoạt</a>' ;
					$button = 	'<div class="btn-group">
									<button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
										Action <span class="caret"></span>
									</button>
									<ul class="dropdown-menu">
										<li><a href="javascript:;" class="edit-user" ><i class="fa fa-edit"></i> Sửa</a></li>
										<li>'.$status.'</li>
										<li><a href="javascript:;" class="edit-upass" ><i class="fa fa-lock"></i> Đổi mật khẩu</a></li>
									</ul>
								</div>';
			...
			if($row > 0){
				...
				foreach ($fetch as $key => $value) {
					...
					$button = 	'<div class="btn-group">
										...
										<a href="items_info?item='.$value['e_id'].'&1Qke#%Rasd2#a1Qasd" class="btn btn-primary"><i class="fa fa-search"></i> Thêm thông tin</a>
					...
					if($count > 0){
						...
						foreach ($fetch as $key => $value) {
							...
							$button = 	'<div class="btn-group">
											...
											<ul class="dropdown-menu">
												<li><a href="javascript:;" class="transfer" data-id="'.$value['itemID'].'"><i class="fa fa-forward"></i> Chuyển</a></li>
											</ul>
										</div>';
